...
- **Image offload** (optional, `compression.image_offload_budget_bytes`): top-level image content blocks past the configured byte budget are replaced with a text placeholder instead of being forwarded, avoiding raw provider rejections for oversized inline images. Off by default (`0`) — images continue to pass through untouched unless explicitly configured; images inside `tool_result` blocks are unaffected.
